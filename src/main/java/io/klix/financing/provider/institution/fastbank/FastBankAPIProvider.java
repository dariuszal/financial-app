...
import static io.klix.financing.enums.ProviderEnum.FAST_BANK;

@Service
@Slf4j
public class FastBankAPIProvider extends AbstractProvider<FastBankRequest> {
    private RestTemplate restTemplate;
    private RequestService requestService;

    @Value("${provider.FAST_BANK.expire-after-hours}")
    private int requestExpireInHours;

    public FastBankAPIProvider(
    ...
    public void submitApplications(ProviderAccount providerAccount, ApplicationRequest applicationRequest) {
        ...
        if (providerAccount.getEnabled()) {
            try {
                ...
                FastBankRequest fastBankRequest = buildProviderRequest(applicationRequest);
                ResponseEntity<? extends AbstractProviderApplicationResponse> response =
                        restTemplate.postForEntity(url, fastBankRequest, FastBankApplicationResponse.class);
    ...
    protected void processStatusResponse(Request request, ResponseEntity<? extends AbstractProviderApplicationResponse> response) {
        FastBankApplicationResponse responseBody = (FastBankApplicationResponse) response.getBody();
    ...
    @Override
    protected FastBankRequest buildProviderRequest(ApplicationRequest applicationRequest) {
        return FastBankRequest.builder()
                ...
                .agreeToDataSharing(applicationRequest.getAgreeToDataSharing())
    ...
    protected void persistRequest(ResponseEntity<? extends AbstractProviderApplicationResponse> providerResponse, Request request) {
        try {
            FastBankApplicationResponse response = (FastBankApplicationResponse) providerResponse.getBody();
            ApplicationMapper.mapFastBankApplicationResponseToRequest(response, request);
            requestService.saveRequest(request);
        } catch (Exception e) {
    ...
    private void syncSingleRequestStatus(ProviderAccount providerAccount, Request request) {
        try {
            ...
            ResponseEntity<? extends AbstractProviderApplicationResponse> response = restTemplate.getForEntity(url, FastBankApplicationResponse.class);
    ...
    protected void handleStatusProcessed(Request request, AbstractProviderApplicationResponse responseBody) {
        ...
        FastBankApplicationResponse fastBankResponse = (FastBankApplicationResponse) responseBody;
        if (fastBankResponse.getFastBankOffer() != null) {
            request.setOffer(OfferMapper.mapFastBankOfferAndRequestToOffer(fastBankResponse.getFastBankOffer(), request));
